...
/// <summary>
/// EF Core + PostgreSQL round-trip tests. Require Docker and a running
/// PostgreSQL container via Testcontainers. Skipped automatically when
/// the SKIP_INTEGRATION_TESTS environment variable is set.
/// </summary>
[Trait("Category", "Integration")]
public class EFCorePostgreSqlIntegrationTests : IAsyncLifetime
{
    ...
    private sealed class PostgreSqlDbContext : DbContext
    {
        private readonly string _cs;
        public DbSet<TemporalEntity> Entities { get; set; } = null!;

        public PostgreSqlDbContext(string connectionString) => _cs = connectionString;
    ...
    [Fact]
    public async Task EnforceUtcDateTimes_DateTime_StoredAsUtc_PostgreSql()
    {
        if (_container is null) return; // skipped

        await using var ctx = new PostgreSqlDbContext(_connectionString);
    ...
    [Fact]
    public async Task UseTemporalConventions_DateOnly_RoundTrips_PostgreSql()
    {
        if (_container is null) return; // skipped
    ...
    [Fact]
    public async Task UseTemporalConventions_DateTimeOffset_StoredAsUtc_PostgreSql()
    {
        if (_container is null) return; // skipped
        ...
        entity.UpdatedAt.UtcDateTime.Hour.Should().Be(13);
    }
}
